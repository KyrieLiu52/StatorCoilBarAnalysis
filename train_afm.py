...
def model_load_afm(IMG_SHAPE=(224, 224, 3), class_num=3, learning_rate=0.001, lr_decay_steps=20, lr_decay_rate=1, dropout_rate=0.2):
...
def train_afm(my_callback, data_dir="data/afm/train", model_save_dir="models", epochs=30, batch_size=32,
    ...
    result_img_type_dir = os.path.join(result_train_folder, "afm")
    ...
    model = model_load_afm(class_num=len(class_names), learning_rate=learning_rate, lr_decay_steps=lr_decay_steps, lr_decay_rate=lr_decay_rate, dropout_rate=dropout_rate)
    model.summary()

    best_model_name = "best_mobilenet_AFM" + "_in_" + str(epochs) + "_epochs.h5"  # 保存最好的model文件名的格式
    last_model_name = "last_mobilenet_AFM" + "_in_" + str(epochs) + "_epochs.h5"  # 保存最后一个epoch的model文件名的格式
    ...
    print("Training image type is AFM")
    ...
    if judge_is_copy_model_to_model_dir("AFM", highest_val_acc, highest_acc, best_model_path_in_model_dir):
        shutil.copy(best_model_path, model_dir)  # 把在checkpoint中找到的最佳模型，复制到models目录下（之前保存在每次运行的目录下）

    # 生成一个保存的目录save_dir ,传入绘制图像的函数中
    save_sub_dir = os.path.join("afm", result_runs_folder)
...
if __name__ == "__main__":
    train_afm("data_split/afm/train", epochs=10)
